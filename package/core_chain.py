...
class ChainLink2:
    def __init__(self, blockchain, proto):
        ...
        self.address_token0 = getattr(self, 'weth')                         # set tokens
        self.address_token1 = getattr(self, 'usdt')
        self.address_pool = ''
        ...
        self.tokens_data = {}                                            # print tokens info
        self.pools = {}
        self.pools_fee = [100, 500, 3000, 10000]  # possible fee tiers
        self.pools_data = {}
        self.nonstable_starts = 6
        self.token_list = list(self.tokens.keys())

        print("Filling tokens data...\n")
        for key, value in self.tokens.items():                             # print tokens info
        ...
        for i in range(len(self.token_list)):
            for j in range(max(i + 1, self.nonstable_starts), len(self.token_list)):
                tokenA = self.token_list[i]
                tokenB = self.token_list[j]
                t0 = self.tokens[tokenA]
                t1 = self.tokens[tokenB]
                for fee in self.pools_fee:
                    pool_address = self.contract_factory.functions.getPool(t0, t1, fee).call()
                    if pool_address == "0x0000000000000000000000000000000000000000": continue
                    ...
                    key = (addr0, addr1, fee)
                    if key in self.pools_data: continue
                    self.pools_data[key] = {
                        "address": pool_address,
                        "contract": pool_contract,
                        "spacing": tick_spacing,}
                    print('Pool', tokenA, tokenB, fee, self.pools_data[key]["address"], 'added. Spacing:', tick_spacing)
        print(f"\nTotal pools found: {len(self.pools_data)}\n")
        ...
        for fee in self.pools_fee:
            ...
            tok0_pool_assets = self.get_balance_token(self.contract_token0, address)
            tok1_pool_assets = self.get_balance_token(self.contract_token1, address)
            tokens_pool_assets = tok0_pool_assets * 3000 + tok1_pool_assets
            ...
            self.pools[fee] = {
                ...
                "reversed": reversed,
                "liquidity": liquidity,
                "balance0": tok0_pool_assets,
                "balance1": tok1_pool_assets,
            }
            print('Pool', address, 'added:', fee, 'Spacing/reversed:', tick_spacing, reversed, 'Liquidity:', liquidity, 'Balances:', tok0_pool_assets, tok1_pool_assets)
            print('Assets total in pool (eth in usd):', tokens_pool_assets)
            print('Assets per liquidity unit:', tokens_pool_assets / liquidity)
            print('-'*25)
        print('Init chain layer completed\n')
        '''
    ...
    def scan_token_cex_price(self):
        for i in range(len(self.token_list)):
            if i < self.nonstable_starts:
                self.tokens_data[self.tokens[self.token_list[i]]]["cex_price"] = 1.0
            else:
                pair = self.token_list[i] + 'USDC'
                timeout = 10
                try:
                    snapshot = self.get_binance_snapshot(pair, timeout)
                except Exception as e:
                    print(f"Error fetching price for {pair}: {e}")
                    self.tokens_data[self.tokens[self.token_list[i]]]["cex_price"] = None
                else:
                    self.tokens_data[self.tokens[self.token_list[i]]]["cex_price"] = snapshot
                    print(f"CEX price for {pair}: {snapshot}")
    ...
    @staticmethod
    def get_binance_price(pair, timeout):
        url = "https://api.binance.com/api/v3/ticker/24hr"
        params = {"symbol": pair+"USDC"}
    ...
    def get_balance_token(self, token_cntr, target_addr):
        try:
            ...
            decimals = token_cntr.functions.decimals().call()
            balance = balance_row / 10**decimals
            address = token_cntr.address
            name = token_cntr.functions.name().call()
            symbol = token_cntr.functions.symbol().call()
            print("Balance", address, name, symbol, ':', balance)
            return balance
        return 0
